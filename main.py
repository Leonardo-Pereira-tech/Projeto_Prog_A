...
def desenhar():
    global x1, y1, x2, y2, figura_nova, cor_carregada
    figura = tipo_figura.get()
    if figura == "Retângulo":
        return canvas.create_rectangle(x1,y1,x2,y2,outline=cor_carregada)
    elif figura == "Oval":
        return canvas.create_oval(x1,y1,x2,y2,outline=cor_carregada)
    elif figura == "Linha":
        return canvas.create_line(x1,y1,x2,y2,fill=cor_carregada)
    else:
        r = ((x2 - x1) **2 + (y2 - y1)**2)**0.5
        return canvas.create_oval(x1 - r, y1 - r, x1 + r, y1 + r,outline=cor_carregada)
...
menu = OptionMenu(frame,tipo_figura,"Retângulo","Círculo","Oval","Linha");menu.pack(side=RIGHT)
frame.pack(fill=X)
...
#Interface de cores
cores = Button(frame,text="Escolher Cores",command=escolher_Cor_borda)
cores.pack()
